...
maybe_escalate_review() {
  ...
  # Pass all escalation flags via environment variables to avoid shell
  # interpolation into Python code (script injection prevention).
  local decision
  decision="$(PYTHONPATH="${SCRIPT_DIR}/.." \
    _ESC_ON_INCOMPLETE="$ESCALATE_ON_INCOMPLETE_REQUIRED_CHECKS" \
    _ESC_ON_REQUEST_CHANGES="$ESCALATE_ON_FAST_REQUEST_CHANGES" \
    _ESC_ON_LOW_CONFIDENCE="$ESCALATE_ON_FAST_LOW_CONFIDENCE" \
    _ESC_ON_BLOCKERS="$ESCALATE_ON_TOOL_OR_EVIDENCE_BLOCKERS" \
    _ESC_ON_DIRTY_BASELINE="$ESCALATE_ON_DIRTY_BASELINE" \
    _ESC_ON_PLANNING_FAILURE="$ESCALATE_ON_TOOL_PLANNING_FAILURE" \
    _ESC_DIRTY_BASELINE="$DIRTY_BASELINE" \
    python3 <<'PY' 2>/dev/null || echo no
import os
from pr_reviewer.escalation import should_escalate
escalate, reasons = should_escalate(
    on_incomplete=(os.environ.get('_ESC_ON_INCOMPLETE') == 'true'),
    on_request_changes=(os.environ.get('_ESC_ON_REQUEST_CHANGES') == 'true'),
    on_low_confidence=(os.environ.get('_ESC_ON_LOW_CONFIDENCE') == 'true'),
    on_blockers=(os.environ.get('_ESC_ON_BLOCKERS') == 'true'),
    on_dirty_baseline=(os.environ.get('_ESC_ON_DIRTY_BASELINE') == 'true'),
    on_planning_failure=(os.environ.get('_ESC_ON_PLANNING_FAILURE') == 'true'),
    dirty_baseline=(os.environ.get('_ESC_DIRTY_BASELINE') == 'true'),
)
