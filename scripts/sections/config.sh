...
reassemble_sse_response() {
  ...
  local api_format="$2"
  # Pass values via environment variables to avoid shell interpolation into
  # Python code (script injection prevention).
  PYTHONPATH="${SCRIPT_DIR}/.." \
  _SSE_RESPONSE_FILE="$response_file" \
  _SSE_API_FORMAT="$api_format" \
  python3 <<'PY'
import os
from pr_reviewer.sse_reassembler import reassemble_sse_to_file
reassemble_sse_to_file(os.environ['_SSE_RESPONSE_FILE'], os.environ['_SSE_API_FORMAT'])
PY
}
...
parse_and_validate() {
  ...
  # Python code (script injection prevention).
  PYTHONPATH="${SCRIPT_DIR}/.." \
  _PARSE_RESPONSE_FILE="$response_file" \
  python3 <<'PY' || return 1
import json, os
from pathlib import Path
from pr_reviewer.response_parser import parse_response_file

result = parse_response_file(os.environ['_PARSE_RESPONSE_FILE'])
...
apply_all_enforcement_wrapper() {
  ...
  # Python code (script injection prevention).
  PYTHONPATH="${SCRIPT_DIR}/.." \
  _EVIDENCE_BLOCKER_ENABLED="$evidence_blocker_enabled" \
  _TOOL_FAILURE_ENABLED="$tool_failure_enabled" \
  _TOOL_MIN_SUCCESSFUL="$tool_min_successful" \
  _VERDICT_POLICY="$verdict_policy" \
  _VALIDATE_CHECKS="$validate_checks" \
...
carry_forward = os.environ.get('_CARRY_FORWARD', 'false')
verdict_policy = os.environ.get('_VERDICT_POLICY', 'model')
validate_checks = os.environ.get('_VALIDATE_CHECKS', 'auto')
validation_mode = os.environ.get('_VALIDATION_MODE', 'warn')
evidence_blocker_enabled = os.environ.get('_EVIDENCE_BLOCKER_ENABLED', 'false')
tool_failure_enabled = os.environ.get('_TOOL_FAILURE_ENABLED', 'false')
tool_min_successful_str = os.environ.get('_TOOL_MIN_SUCCESSFUL', '0')
# Validate that tool_min_successful is a safe integer before use
if not tool_min_successful_str.isdigit():
    tool_min_successful_str = '0'
tool_min_successful = int(tool_min_successful_str)
